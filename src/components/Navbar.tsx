        ...
        {isAuthenticated && user ? (
          <div className="flex items-center gap-6">
            <div className="hidden md:flex items-center gap-1.5 px-3 py-1.5 bg-white/5 border border-white/10 rounded-full">
              <span className="dt-mono text-[10px] text-white/40 uppercase tracking-widest">
                Online
              </span>
            ...
            <div className="relative" ref={dropdownRef}>
              <button
                ...
                onClick={() => {
                  console.log(
                    "Dropdown button clicked, prev state:",
                    isDropdownOpen,
                  );
              ...
              {isDropdownOpen && (
                <div className="absolute right-0 mt-3 w-56 rounded-2xl bg-[#141416] border border-white/10 shadow-2xl py-2 animate-in fade-in slide-in-from-top-4 duration-300 z-50 overflow-hidden">
                  <div className="px-4 py-3 mb-1 border-b border-white/5 bg-white/[0.02]">
                    <p className="dt-mono text-[10px] text-white/30 uppercase tracking-wider mb-1">
                      Signed in as
                    </p>
                    <p className="text-sm font-semibold text-[#f0ede8] truncate">
                      {user.emailId}
                    </p>
                  </div>
          ...
          <div className="flex items-center gap-4">
            <Link
              href="/login"
              className="text-sm font-bold text-white/60 hover:text-[#DC2626] transition-colors"
            >
              Sign In
            </Link>
            <Link
