...
export default function FeedCarousel({ feed: initialFeed }: FeedCarouselProps) {
  // Read from useFeedStore — same store useFeed populates and useSendRequest drains
  const storeFeed = useFeedStore((state) => state.feed);
  const displayFeed = storeFeed ?? initialFeed;
